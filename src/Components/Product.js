  ...
  return (
    <div className='row mt-3'>
        ...
        <div class="btn-group" role="group" aria-label="Basic mixed styles example">
            ...
            <button type="button" class="btn btn-danger" onClick={() => {props.incrementQuantity(props.index)}}>+</button>
        </div>
        </div>
        <div className='col-2'>
            {props.product.quantity * props.product.price}
        </div>
        <button className='col-2 btn btn-danger mb-4 mt-4' onClick={() => {props.removeItem(props.index)}}>
          Remove
        </button>
    </div>
  )
